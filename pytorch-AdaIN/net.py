...
class NetFaceSegment(nn.Module):
    def __init__(self, encoder, decoder, face_net):
        super(NetFaceSegment, self).__init__()
        enc_layers = list(encoder.children())
        n_classes = 19
    ...
    def calc_face_segmentation_loss(self, input, target):
        assert (input.size() == target.size())
        assert (target.requires_grad is False)
        lossL2 = L2Loss()
        return lossL2(input, target)
    ...
    def forward(self, content, style, alpha=1.0):
        ...
        style_feats = self.encode_with_intermediate(style)
        content_feat = self.encode(content)
        content_face_segmented = self.face_net(content)[0]
        ...
        g_t = self.decoder(t)
        g_t_feats = self.encode_with_intermediate(g_t)
        stylized_face_segmented = self.face_net(g_t)[0]
 
        

        loss_c = self.calc_content_loss(g_t_feats[-1], t)
        loss_s = self.calc_style_loss(g_t_feats[0], style_feats[0])
        loss_f = self.calc_face_segmentation_loss(stylized_face_segmented, content_face_segmented)
            
        for i in range(1, 4):
            loss_s += self.calc_style_loss(g_t_feats[i], style_feats[i])
        return loss_c, loss_s, loss_f, stylized_face_segmented, content_face_segmented
